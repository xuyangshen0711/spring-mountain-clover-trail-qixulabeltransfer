...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    // Same files PGLite applies. Deployed DBs previously skipped this and
    // then saveStyle failed with "relation app_meta does not exist".
...
async function createPgliteSql(): Promise<Sql> {
  ...
  const migrate = async (): Promise<void> => {
    // Eager glob so adding migrations/*.sql (e.g. 0004_style_assets_compat) hot-reloads.
    const migrations = import.meta.glob("/migrations/*.sql", {
      query: "?raw",
      import: "default",
      eager: true,
 ...
 *
 * Opens the client and applies `migrations/*.sql` on **both** PGLite and Neon.
